...
class Redmine::ApiTest::ResourceBookingsTest < ActiveRecord::VERSION::MAJOR >= 4 ? Redmine::ApiTest::Base : ActionController::IntegrationTest
  ...
  test "PUT /resource_bookings/:id.xml with failed update" do
    ...
    assert_select 'errors error', :text => "Hours/Day cannot be blank"
  end
  ...
  test "PUT /resource_bookings/:id.json with failed update" do
    ...
    assert_response :unprocessable_entity
    json = ActiveSupport::JSON.decode(response.body)
    assert json['errors'].include?("Hours/Day cannot be blank")
  end
